...
std::vector<Mat> TrackerCSRT::get_features(const Mat &patch, const Size2i &feature_size)
{
	...
	if(params.use_gray) {
		...
		//cv::imshow("gray_m", gray_m);
	}
	if(params.use_rgb) {
		std::vector<Mat> rgb_features;
		get_features_rgb(patch, feature_size, rgb_features);
...
std::vector<Mat> TrackerCSRT::get_features(const Mat &patch, const Size2i &feature_size)
{
	...
	if (params.use_hog) {
		features.insert(features.end(), hogFeatures.begin(), hogFeatures.begin()+params.num_hog_channels_used);
	}
	if (params.use_color_names) {
...
bool TrackerCSRT::initImpl(const Mat& image_, const Rect2d& boundingBox)
{
	...
	cell_size = cvFloor(std::min(4.0, std::max(1.0, static_cast<double>(cvCeil((bounding_box.width * bounding_box.height)/400.0)))));
	original_target_size = Size(bounding_box.size());

	template_size.width = static_cast<float>(
			cvFloor(original_target_size.width + params.padding * sqrt(original_target_size.width * original_target_size.height)));

	template_size.height = static_cast<float>(
			cvFloor(original_target_size.height + params.padding * sqrt(original_target_size.width * original_target_size.height)));

	template_size.width = template_size.height = (template_size.width + template_size.height) / 2.0f;
	...
	rescaled_template_size = Size2i(cvFloor(template_size.width * rescale_ratio), cvFloor(template_size.height * rescale_ratio));
	...
	//cv::imshow("default_mask", default_mask);
	//initialize filter
