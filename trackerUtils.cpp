...
void get_features_cn(const Mat &ppatch_data, const Size &output_size, std::vector<Mat> &features)
{
	...
	split(cnFeatures, features);
	for (size_t i = 0; i < features.size(); i++) {
		if (output_size.width > 0 && output_size.height > 0) {
			resize(features.at(i), features.at(i), output_size, INTER_CUBIC);
		}
	}
}
...
{
	split(patch, features);
	for(size_t k=0; k<features.size(); k++) {
		features[k].convertTo(features[k], CV_32F, 1.0/255.0, -0.5);
		features[k] = features[k] - mean(features[k])[0];
		resize(features[k], features[k], output_size, INTER_CUBIC);
	}
}
...
{
	Mat channel;
	M.convertTo(channel, CV_32F);
	dft(channel, channel, DFT_COMPLEX_OUTPUT);
	out = (channel);
}

void ttt(const Mat &M, Mat &out)
{
...
std::vector<Mat> fourier_transform_features_(const std::vector<Mat> &M)
{
	...
	for(size_t k = 0; k < M.size(); k++) {
		threads.push_back(std::thread(ttt, std::ref(M[k]), std::ref(out[k])));
		//fourier_transform_mat(M[k], out[k]);
	}
